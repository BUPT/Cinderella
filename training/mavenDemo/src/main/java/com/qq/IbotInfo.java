...
public class IbotInfo extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String filename = "D:\\iBotest\\bizplan1.txt";
		...
		try {
			botResult = GetKeyPart(filename,botResult);
			botResult = GetOtherPart(filename,botResult);
	...
	/**
	 * 从request中提取出采用curl -d 传输的json信息并进行UTF-8编码
	 * @param request
	 * @return json信息字符串
	 * @throws IOException
	 */
	...
	/**
	 * 从email对象中获取邮件信息，并以txt格式存储
	 * @param fileName 存储邮件信息的文件名
	 * @param email email对象
	 */
	...
	/**
	 * 获取BP语料的其他信息，包括Location、founderName、company_name、product_name
	 * @param infileName
	 * @param bot
	 * @return
	 * @throws IOException
	...
	/**
	 * 将txt信息转化成字符串发送给BosonNLP进行处理
	 * @param infileName 存储邮件信息的文件名
	 * @param SENTIMENT_URL BosonNLP的API地址
	 * @return
	 * @throws IOException
	 * @throws UnirestException
	 */
